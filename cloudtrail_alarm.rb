...
class CloudTrailAlarm
  def initialize
    ...
    @incap_api_id = config["incapsula"]["api_id"]
    @incap_api_key = config["incapsula"]["api_key"]
    ...
    @iam = Aws::IAM::Client.new(credentials: creds, region: $region)
    @sns = Aws::SNS::Client.new(credentials: creds, region: $region)
    @cloudwatchlogs = Aws::CloudWatchLogs::Client.new(credentials: creds, region: $region)
    @cloudwatch = Aws::CloudWatch::Client.new(credentials: creds, region: $region)
